...
const PreferdParties = ({campainId}) => {
    ...
    const getCampains = () => {
      setLoading(true)
        ...
        let URL = `${import.meta.env.VITE_API_SERVER}/api/v1/reports/parties?campainId=${campainId}`;
...
let nameMunicipality = data?.municipalities?.name.replace(/[áéíóúÁÉÍÓÚ]/g, (letra) => acentos[letra] || letra);
function convertToExcel(data) {
    ...
    function handleExportToExcel() {
      const partyData = data.encuestas.map((voto) => {
        return {
      ...
      XLSX.writeFile(wb, 'informe Partido Preferido por Municipio.xlsx');
    }
...
const resultArray = Object.values(partyData);
  ...
  <div className="card-header border-0">
    <div className="d-flex justify-content-between">
      <h3 className="card-title"> {data.name} <small>Métricas de Partidos por Lugares</small> </h3>
  ...
  <div className="card-body">
      ...
      <div className="d-flex flex-column">
        <span className="text-bold text-lg"> {data.name}</span>
      ...
      {
        data.name?<button className='btn btn-app bg-success' onClick={handleExportToExcel}><i className="fas fa-file-excel"></i>Exportar a Excel</button>:""
      }
