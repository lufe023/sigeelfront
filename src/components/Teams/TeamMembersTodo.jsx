...
const [isLoading, setIsLoading] = useState(false)
  const getPeopleById = (userId)=>{
    setIsLoading(true)
    const URL = `${import.meta.env.VITE_API_SERVER}/api/v1/census/simpleCensus`
        axios.post(URL,
          {
            leaderId:userId
        },
...
<div className='row'>
  ...
  <div className="card card-primary card-outline" style={{borderTop:`3px solid ${randomColor}`}}>
    ...
    <div className="card collapsed-card card-primary card-outline" style={{borderTop:`3px solid ${randomColor}`}}>
    <div className="card-header">
...
<div className="card collapsed-card">
      <div className="card-header">
  <h3 className="card-title">Padroncillo</h3>
  <div className="card-tools">
    <button type="button" className="btn btn-tool" data-card-widget="collapse">
      <i className="fas fa-plus" />
    </button>
  ...
  <div className="card-body p-0">
      ...
      
    {
        personas?.map(persona => 
          <li className='simpleCensu' key={persona.id} style={{border:"1px solid #DCDCDC", borderRadius:"10px",margin:"5px 3px"}}>
            <style>{`
    .simpleCensu:hover {
      background-color: #F5F5F5;
    }
  `}</style>
          <Link to={`/mypeople/${persona.id}`}>
          <img src={`${import.meta.env.VITE_API_SERVER}/api/v1/images/citizen/${persona?.picture}`} alt="User Image" /></Link>
          <Link to={`/mypeople/${persona.id}`} className="users-list-name" href="#">{persona.firstName}</Link>
          {persona?.sufragio?.suffrage?<span className="users-list-date bg-success">Votó</span>:
          <span className="users-list-date bg-danger">No votó</span>
          }
          <b>Mesa:</b> {persona.colegio.collegeNumber}<br/>
          <small style={{lineHeight:"0"}}>{persona.colegio.precinctData.recintoNombre}</small>
          </li>
        )
      }
    </ul>
...
{
  isLoading?<div className='loading' style={{height:"100px", marginBottom:"50px"}}><Cargando escala='1.5'/></div>:""
}
  {
      personas?"":
      <button className='btn btn-primary btn-block' onClick={()=>getPeopleById(member.memberId)}>Ver</button>
    }
  </div>
  {/* /.card-footer */}
</div>
